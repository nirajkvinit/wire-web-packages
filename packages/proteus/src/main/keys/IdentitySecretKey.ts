...
export class IdentitySecretKey {
  ...
  static new(sec_edward: Uint8Array, sec_curve: Uint8Array): IdentitySecretKey {
    const sk = ClassUtil.new_instance(IdentitySecretKey);
  ...
  static deserialise(buf: ArrayBuffer): IdentitySecretKey {
    const decoder = new CBOR.Decoder(buf);
    return IdentitySecretKey.decode(decoder);
  }

  encode(encoder: CBOR.Encoder): CBOR.Encoder {
    encoder.object(2);
    encoder.u8(0);
    encoder.bytes(this.sec_edward);
    encoder.u8(1);
    return encoder.bytes(this.sec_curve);
  }

  static decode(decoder: CBOR.Decoder): IdentitySecretKey {
    const self = ClassUtil.new_instance(IdentitySecretKey);
    ...
    if (sec_edward) {
      self.sec_edward = sec_edward;
      if (!sec_curve) {
        self.sec_curve = sodium.crypto_sign_ed25519_sk_to_curve25519(sec_edward);
      } else {
